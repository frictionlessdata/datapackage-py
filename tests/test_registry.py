...
EMPTY_REGISTRY_PATH = 'data/empty_registry.json'
BASE_AND_TABULAR_REGISTRY_PATH = 'data/base_and_tabular_registry.json'
UNICODE_REGISTRY_PATH = 'data/unicode_registry.json'
BASE_PROFILE_PATH = 'data/base_profile.json'
...
@httpretty.activate
def test_init_raises_if_registry_has_no_id_field():
...
@httpretty.activate
def test_init_raises_if_registry_url_doesnt_exist():
    url = 'http://some-place.com/registry.csv'
...
def test_available_profiles_works_with_unicode_strings_in_registry():
    registry_path = UNICODE_REGISTRY_PATH
    registry = datapackage.registry.Registry(registry_path)
    assert len(registry.available_profiles) == 2
...
def test_base_path_defaults_to_the_local_cache_path():
    registry = datapackage.registry.Registry()
...
@pytest.mark.skip('deprecated')
def test_base_path_uses_received_registry_base_path():
    registry = datapackage.registry.Registry(EMPTY_REGISTRY_PATH)
    base_path = os.path.dirname(EMPTY_REGISTRY_PATH)
    assert registry.base_path == base_path
...
@httpretty.activate
def test_base_path_cant_be_set():
    registry = datapackage.registry.Registry()
    with pytest.raises(AttributeError):
